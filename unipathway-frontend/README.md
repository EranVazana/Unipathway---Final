...
### 🤖 AI Advisor (Gemini)
New **AI Advisor** tab on the Home page — a chat interface backed by `POST /api/ai/chat`. All AI calls go through the backend; the Gemini API key is never exposed to the frontend. The advisor knows every university, department, and threshold in the system plus your own Sekem scores.

### 🔔 Live Notifications (Socket.IO)
A notification bell in the navbar receives real-time events over WebSockets (`socket.io-client`). When an admin creates a new university or department, all other logged-in clients see it instantly — try it with two browser windows logged in as different users. Notifications persist in MySQL with read/unread state.

New frontend pieces: `src/context/NotificationContext.jsx` (socket lifecycle + state), `src/components/ui/NotificationBell.jsx`, `src/components/views/AIChatView.jsx`, `src/services/aiService.js`.

### Backend requirement
This frontend now requires the Assignment 4 backend (MySQL + Socket.IO + Gemini) running at `http://localhost:3000`.
